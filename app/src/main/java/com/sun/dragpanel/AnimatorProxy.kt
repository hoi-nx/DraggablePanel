...
class AnimatorProxy private constructor(view: View) : Animation() {
    ...
    private fun invalidateAfterUpdate() {
        ...
        (view.parent as View).invalidate(
            floor(after.left.toDouble()).toInt(),
            floor(after.top.toDouble()).toInt(),
            ceil(after.right.toDouble()).toInt(),
            ceil(after.bottom.toDouble()).toInt()
        )
    }
    ...
    companion object {
        ...
        val NEEDS_PROXY = Integer.valueOf(Build.VERSION.SDK) < Build.VERSION_CODES.HONEYCOMB
