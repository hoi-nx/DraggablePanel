 ...
 */
(private val draggableView: DraggableView, private val draggedView: View) : ViewDragHelper.Callback() {
    ...
    override fun clampViewPositionHorizontal(child: View, left: Int, dx: Int): Int {
        var newLeft = draggedView.getLeft()
    ...
    override fun clampViewPositionVertical(child: View, top: Int, dy: Int): Int {
        var newTop = draggableView.getHeight() - draggableView.draggedViewHeightPlusMarginTop
        if (draggableView.isMinimized && Math.abs(dy) >= MINIMUM_DY_FOR_VERTICAL_DRAG || !draggableView.isMinimized && !draggableView.isDragViewAtBottom) {
            ...
            val topBound = draggableView.getPaddingTop()
            val bottomBound = (draggableView.getHeight()
    ...
    private fun triggerOnReleaseActionsWhileHorizontalDrag(xVel: Float) {
        if (xVel < 0 && xVel <= -X_MIN_VELOCITY) {
            ...
            when {
                draggableView.isNextToLeftBound -> draggableView.closeToLeft()
                draggableView.isNextToRightBound -> draggableView.closeToRight()
                else -> draggableView.minimize()
            }
        }
    }
